...
const SEARCH_DIRS = ["packages/react/src", "packages/docs/stories"];
...
function migrateFile(filePath) {
  ...
  Object.entries(SPACE_MAPPING).forEach(([oldPattern, newPattern]) => {
    ...
    if (matches) {
      ...
      changes.push(
        `theme.space${oldPattern} → theme.space${newPattern} (${matches.length} times)`,
      );
      changesMade = true;
    }
  });
  ...
  Object.entries(ELEVATION_MAPPING).forEach(([oldPattern, newPattern]) => {
    ...
    if (matches) {
      ...
      changes.push(
        `theme.elevation${oldPattern} → theme.elevation${newPattern} (${matches.length} times)`,
      );
      changesMade = true;
    }
  });
