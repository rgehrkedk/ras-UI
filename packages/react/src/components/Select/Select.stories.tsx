...
const BasicSelectTemplate = (args: SelectProps<object>) => {
  ...
  return (
    <div style={{ width: '300px' }}>
      ...
      <p style={{ marginTop: '1rem', fontSize: '14px', color: 'var(--color-semantic-text-secondary)' }}>
...
export const FullWidth: Story = {
  render: (args) => (
    <div style={{ width: '400px', padding: '20px', border: '1px dashed var(--color-border-default)' }}>
  ...
  return (
    <div style={{ width: '300px' }}>
      <Select 
        ...
        onSelectionChange={setSelectedKey}
      >
        <SelectItem id="home" icon={<HomeIcon />}>Home</SelectItem>
        <SelectItem id="profile" icon={<UserIcon />}>Profile</SelectItem>
        <SelectItem id="settings" icon={<SettingsIcon />}>Settings</SelectItem>
        <SelectItem id="messages" icon={<MailIcon />}>Messages</SelectItem>
      </Select>
      <p style={{ marginTop: '1rem', fontSize: '14px', color: 'var(--color-semantic-text-secondary)' }}>
        Selected: {selectedKey || 'None'}
      </p>
  ...
  return (
    <div style={{ width: '350px' }}>
        ...
        {(user) => (
          ...
          >
            <div>
              ...
              <div style={{ fontSize: '12px', color: 'var(--color-semantic-text-secondary)', marginTop: '2px' }}>
      ...
      <p style={{ marginTop: '1rem', fontSize: '14px', color: '#666' }}>
        Selected: {selectedKey ? users.find(u => u.id === selectedKey)?.name : 'None'}
      </p>
    </div>
  );
};
...
const ActionsTemplate = (args: SelectProps<object>) => {
  ...
  return (
    <div style={{ width: '300px' }}>
      ...
      >
        <SelectItem id="edit" icon={<EditIcon />}>Edit Item</SelectItem>
        <SelectItem id="duplicate" icon={<UserIcon />}>Duplicate</SelectItem>
        <SelectItem id="delete" icon={<TrashIcon />}>Delete Item</SelectItem>
      </Select>
      {message && (
        <p style={{ 
          marginTop: '1rem', 
  ...
  return (
    <form onSubmit={handleSubmit} style={{ width: '300px', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <Select
        ...
        errorMessage={submitted && !formData.country ? "Country is required" : undefined}
        selectedKey={formData.country}
        onSelectionChange={(key) => setFormData(prev => ({ ...prev, country: key }))}
      >
        <SelectItem id="us">United States</SelectItem>
        <SelectItem id="ca">Canada</SelectItem>
        <SelectItem id="uk">United Kingdom</SelectItem>
      ...
      <Select
        ...
        errorMessage={submitted && !formData.role ? "Role is required" : undefined}
        selectedKey={formData.role}
        onSelectionChange={(key) => setFormData(prev => ({ ...prev, role: key }))}
      >
        <SelectItem id="admin" icon={<SettingsIcon />}>Administrator</SelectItem>
        <SelectItem id="editor" icon={<EditIcon />}>Editor</SelectItem>
        <SelectItem id="user" icon={<UserIcon />}>User</SelectItem>
      </Select>
      ...
      <Select
        ...
        errorMessage={submitted && !formData.status ? "Status is required" : undefined}
        selectedKey={formData.status}
        onSelectionChange={(key) => setFormData(prev => ({ ...prev, status: key }))}
      >
      ...
      <button 
        ...
        style={{ 
          ...
          backgroundColor: isValid ? 'var(--color-components-button-primary-background)' : 'var(--color-components-button-secondary-background)',
          color: isValid ? 'var(--color-components-button-primary-text)' : 'var(--color-components-button-secondary-text)',
          border: 'none',
          borderRadius: '6px',
      ...
      {submitted && (
        <p style={{ fontSize: '14px', color: 'var(--color-semantic-success)', marginTop: '0.5rem' }}>
