...
export type BreadcrumbSeparator = 'chevron-right' | 'arrow-right' | 'slash' | React.ReactNode;
...
const BreadcrumbEllipsisMenu: React.FC<{
  ...
  return (
    <MenuTrigger>
      <Button className={breadcrumbEllipsisButton({ size })} aria-label="Show more breadcrumbs">
...
export const Breadcrumbs = React.forwardRef<HTMLOListElement, BreadcrumbsProps>(
  (
    ...
    if (!items || items.length === 0) {
      console.warn('Breadcrumbs: items prop is required and must contain at least one item');
      return null;
    }
    ...
    const { visibleItems, collapsedItems } = useMemo(() => {
      ...
      const collapsedItems = items.slice(1, -(numLastItems));
              ...
              <React.Fragment key={item.id}>
                ...
                {needsEllipsis && (
                  <>
                    {showSeparators && (
                      <li>
                        <BreadcrumbSeparator separator={separator} size={size} />
                      </li>
                    )}
                    <li>
                      <div className={breadcrumbEllipsis({ size })}>
                        <BreadcrumbEllipsisMenu items={collapsedItems} size={size} />
                      </div>
                    </li>
                  </>
                )}
