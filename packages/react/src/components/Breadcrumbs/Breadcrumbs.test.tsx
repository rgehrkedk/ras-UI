...
describe("Breadcrumbs", () => {
  ...
  describe("Auto-collapse Functionality", () => {
    test("shows all items when count is within maxItems limit", () => {
      ...
      expect(
        screen.queryByLabelText(/show more breadcrumbs/i),
    ...
    test("collapses items when maxItems is exceeded", () => {
      ...
      expect(
        screen.getByLabelText(/show more breadcrumbs/i),
      ).toBeInTheDocument();
    ...
    test("ellipsis menu shows collapsed items when clicked", async () => {
      ...
      expect(
        screen.getByRole("menuitem", { name: "Subcategory" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitem", { name: "Product Type" }),
      ).toBeInTheDocument();
    });
  });
    ...
    test("renders different sizes correctly", () => {
      ...
      sizes.forEach((size) => {
        const { container } = render(
          <Breadcrumbs items={mockItems} size={size} />,
        );
        const nav = container.querySelector("nav");
    ...
    test("accepts custom aria-label", () => {
      ...
      expect(
        screen.getByRole("navigation", {
          name: /custom navigation breadcrumb/i,
        }),
      ).toBeInTheDocument();
    });
  ...
  describe("Responsive Behavior", () => {
    test("applies text overflow classes for long labels", () => {
      const longLabelItems = [
        {
          id: "1",
          label: "Very Long Category Name That Should Truncate",
          href: "/",
        },
  ...
  describe("Edge Cases", () => {
    test("handles single item breadcrumb", () => {
      ...
      expect(screen.getByText("Single Page")).toHaveAttribute(
        "aria-current",
        "page",
      );
