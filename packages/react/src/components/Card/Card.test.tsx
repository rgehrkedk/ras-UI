...
describe("Card Integration", () => {
  it("renders complete card structure", () => {
    ...
    expect(
      screen.getByText("This is a description of the card content."),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Main content goes here with various elements."),
    ).toBeInTheDocument();
  ...
  it("handles partial card structures", () => {
    ...
    expect(
      screen.getByRole("heading", { level: 3, name: "Just a title" }),
    ).toBeInTheDocument();
  ...
  it("handles nested interactive content", async () => {
    ...
    const card =
      screen.getByText("Interactive Card").closest('[role="generic"]') ||
      screen.getByText("Content with interaction").closest("div")
        ?.parentElement;
