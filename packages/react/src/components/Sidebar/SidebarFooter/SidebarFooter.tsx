...
export const SidebarFooter = React.forwardRef<HTMLDivElement, SidebarFooterProps>(
  ({ children, userName, userEmail, avatar, className, ...props }, ref) => {
    ...
    if (userName || userEmail) {
      const finalAvatar = avatar || <Avatar name={userName || 'User'} size="md" variant="primary" />;
      ...
      return (
        ...
        >
          ...
          <div>
            {userName && <div className={styles.sidebarFooterName}>{userName}</div>}
            {userEmail && <div className={styles.sidebarFooterEmail}>{userEmail}</div>}
          </div>
        </div>
      );
    }
    ...
    if (children) {
      ...
      return (
        ...
        >
          ...
          {textElement && React.isValidElement(textElement) && (
            React.cloneElement(textElement as React.ReactElement<any>, {}, 
              React.Children.map(textElement.props.children, (child, index) => {
                if (React.isValidElement(child) && child.type === 'div') {
                  const className = index === 0 ? styles.sidebarFooterName : styles.sidebarFooterEmail;
