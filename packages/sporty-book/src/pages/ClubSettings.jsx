...
import { Settings, CreditCard, Users, Shield, Crown, Star, Zap, Plus, LogIn } from "lucide-react"; // Added LogIn
...
export default function ClubSettings() {
  ...
  const loadSettingsData = async () => {
    try {
      ...
      const subscriptions = await ClubSubscription.filter({ club_id: currentUser.club_id });
  ...
  const getPlanIcon = (planType) => {
    switch (planType) {
      case "freemium": return <Zap className="w-5 h-5" />;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-slate-600">You are not associated with any club.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Club Settings</h1>
          <p className="text-slate-600">Manage subscription and team for {club.name}</p>
        </div>
                ...
                {subscription ? (
                  <div className="space-y-6">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 bg-gradient-to-r ${getPlanColor(subscription.plan_type)} rounded-xl flex items-center justify-center text-white`}>
                        {getPlanIcon(subscription.plan_type)}
                      </div>
                      <div>
                        <h3 className="text-xl font-bold text-slate-900 capitalize">{subscription.plan_type} Plan</h3>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-slate-600 mb-4">Subscribe to a plan to unlock premium features.</p>
              ...
              <CardContent>
                {clubStaff.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-xl font-semibold text-slate-900 mb-2">No team members</h3>
                    <p className="text-slate-600">Invite staff members to help manage your club.</p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-gradient-to-r from-slate-500 to-slate-600 rounded-full flex items-center justify-center text-white font-semibold">
                              {staff.full_name?.charAt(0).toUpperCase() || staff.email.charAt(0).toUpperCase()}
                            </div>
                            <div>
                              <h4 className="font-semibold text-slate-900">{staff.full_name || staff.email}</h4>
                              <p className="text-sm text-slate-600">{staff.email}</p>
                            </div>
                          </div>
                          
                          <div className="flex items-center gap-4">
                            <Badge className={staff.club_role === "admin" ? "bg-purple-100 text-purple-800" : "bg-blue-100 text-blue-800"}>
                              <Shield className="w-3 h-3 mr-1" />
                              {staff.club_role || "coach"}
                            </Badge>
                            
                            {staff.id !== user.id && (
                              <Select value={staff.club_role || "coach"} onValueChange={(value) => updateUserRole(staff.id, value)}>
