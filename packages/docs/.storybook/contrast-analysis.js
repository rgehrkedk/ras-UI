...
Object.entries(buttonColors).forEach(([theme, variants]) => {
  ...
  Object.entries(variants).forEach(([variant, colors]) => {
    ...
    console.log(
      `${status} ${variant.padEnd(10)} | ${ratio.toFixed(2)}:1 (${wcagLevel}) | ${background} → ${colors.text}`,
    );
  });
...
if (issues.length === 0) {
  ...
  issues.forEach((issue) => {
    console.log(
      `❌ ${issue.theme}/${issue.variant}: ${issue.ratio.toFixed(2)}:1 (${issue.wcag}) - ${issue.background} → ${issue.text}`,
    );
  });
}
...
console.log("─".repeat(50));
console.log(
  "• brand.500 (#3b82f6) → white:",
  getContrastRatio("#3b82f6", "#ffffff").toFixed(2) + ":1",
);
console.log(
  "• brand.600 (#2563eb) → white:",
  getContrastRatio("#2563eb", "#ffffff").toFixed(2) + ":1",
);
console.log("• Tokens use brand.600 (#2563eb) ✅");
console.log(
  "• A11y addon may be detecting brand.500 (#3b82f6) due to CSS conflicts",
);
