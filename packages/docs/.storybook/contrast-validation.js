...
(function contrastValidation() {
  ...
  contrastDisplays.forEach((display, index) => {
    ...
    console.log(
      `Display ${index + 1} position: top=${rect.top}px, right=${window.innerWidth - rect.right}px`,
    );
  });
  ...
  buttons.forEach((button, index) => {
    ...
    if (bgColor.startsWith("rgb")) {
      ...
      if (hexBg === "#3b82f6") {
        console.warn(
          `⚠️  Button ${index + 1} uses brand.500 (#3b82f6) - this may cause a11y conflicts`,
        );
      } else if (hexBg === "#2563eb") {
        console.log(
          `✅ Button ${index + 1} correctly uses brand.600 (#2563eb)`,
        );
      }
    }
  });
  ...
  const buttonPrimaryBg = getComputedStyle(rootElement)
    .getPropertyValue("--color-components-button-primary-background")
    .trim();
  ...
  console.log("─".repeat(50));
  console.log(
    "1. Ensure all contrast displays have proper exclusion attributes",
  );
