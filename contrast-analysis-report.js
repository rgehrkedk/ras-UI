...
function analyzeContrastCombinations() {
  console.log(
    "🔍 ras-UI Design System - Comprehensive Color Contrast Analysis\n",
  );
        ...
        if (ratio !== null) {
          ...
          if (evaluation.status === "poor") {
            ...
            const improvement = Math.ceil(
              (minRatio / evaluation.ratio - 1) * 100,
            );
            console.log(
              `   ⚠️  Needs ${minRatio}:1 minimum, improve by ${improvement}%`,
            );
          }
...
function analyzeBrandContrasts() {
  ...
  for (const [brandName, brandColors] of Object.entries(brands)) {
    ...
    if (ratio !== null) {
      ...
      console.log(
        `${statusIcon} Primary button contrast: ${evaluation.ratio}:1 (${evaluation.level})`,
      );
      console.log(
        `   White text (${primaryText}) on ${brandName} ${primaryBg}`,
      );
    }
    ...
    if (focusRatio !== null) {
      ...
      console.log(
        `${focusIcon} Focus ring contrast: ${focusEvaluation.ratio}:1`,
      );
      console.log(
        `   ${brandName} focus (${focusColor}) on white (${lightBg})`,
      );
...
function generateSummaryAndRecommendations(results) {
  ...
  if (Object.keys(issuesByTheme).length > 0) {
    ...
    for (const [theme, issues] of Object.entries(issuesByTheme)) {
      ...
      issues.forEach((issue) => {
        ...
        console.log(
          `   - ${issue.combination}: ${issue.ratio}:1${criticalFlag}`,
        );
      });
      console.log("");
    }
  ...
  if (criticalFailures.length > 0) {
    ...
    criticalFailures.forEach((issue, index) => {
      ...
      console.log(
        `   Current: ${issue.ratio}:1, Required: ${WCAG_AA_NORMAL}:1`,
      );
      ...
      if (issue.ratio < 2) {
        console.log(
          `   🎨 Major adjustment needed - consider completely different colors`,
        );
      } else if (issue.ratio < 3.5) {
        console.log(
          `   🎨 Try 2-3 shades darker for text or lighter for background`,
        );
      } else {
  ...
  console.log(
    '  - Shadow tokens should use "shadow" type instead of "boxShadow"',
  );
  console.log(
    '  - Font size tokens should use "fontSize" instead of "dimension"',
  );
  console.log("");
  ...
  console.log(
    "  - Consider forced colors mode support (Windows High Contrast)",
  );
