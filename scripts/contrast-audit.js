...
function loadTokens() {
  ...
  try {
    const core = JSON.parse(readFileSync(resolve(tokensPath, 'tokens/core.json'), 'utf8'));
    const semantic = JSON.parse(readFileSync(resolve(tokensPath, 'tokens/semantic.json'), 'utf8'));
    const components = JSON.parse(readFileSync(resolve(tokensPath, 'tokens/components.json'), 'utf8'));
    
    // Load brand tokens
    const brands = {
      default: JSON.parse(readFileSync(resolve(tokensPath, 'brands/default/core.json'), 'utf8')),
      vibrant: JSON.parse(readFileSync(resolve(tokensPath, 'brands/vibrant/core.json'), 'utf8')),
      corporate: JSON.parse(readFileSync(resolve(tokensPath, 'brands/corporate/core.json'), 'utf8'))
    };
...
function resolveTokenValue(value, context, maxDepth = 5) {
    ...
    if (current && typeof current === 'object' && segment in current) {
      ...
      if (context.color && context.color.base) {
        ...
        for (const baseSegment of basePath) {
          if (baseCurrent && typeof baseCurrent === 'object' && baseSegment in baseCurrent) {
        ...
        const baseValue = typeof baseCurrent === 'object' && baseCurrent.value ? baseCurrent.value : baseCurrent;
        return baseValue;
      }
      return value; // Unable to resolve
    }
  ...
  const resolvedValue = typeof current === 'object' && current.value ? current.value : current;
...
function analyzeTextContrasts(tokens) {
  ...
  for (const theme of themes) {
    for (const combo of combinations) {
      ...
      const backgroundColor = getTokenValue(combo.backgroundToken, theme, tokens);
...
function getTokenValue(tokenPath, theme, tokens) {
  ...
  for (const segment of tokenPath) {
    if (current && typeof current === 'object' && segment in current) {
      ...
      console.log(`Token path not found: ${tokenPath.join('.')} at segment: ${segment}`);
      return null;
    }
  }
  ...
  if (current && typeof current === 'object' && theme in current) {
    ...
    if (themeToken && themeToken.value) {
      ...
      console.log(`Resolved ${tokenPath.join('.')}.${theme}: ${themeToken.value} -> ${resolved}`);
      return resolved;
    }
  }
  ...
  if (current && typeof current === 'object' && current.value) {
    ...
    console.log(`Resolved ${tokenPath.join('.')}: ${current.value} -> ${resolved}`);
    return resolved;
  }

  console.log(`No value found for token: ${tokenPath.join('.')} (theme: ${theme})`);
  return current;
}
...
function analyzeFocusContrasts(tokens) {
  ...
  for (const theme of themes) {
    const focusColor = getTokenValue(['color', 'semantic', 'border', 'focus'], theme, tokens);
    const baseBackground = getTokenValue(['color', 'semantic', 'surface', 'base'], theme, tokens);
...
function generateReport(tokens) {
  ...
  for (const [theme, results] of Object.entries(groupedByTheme)) {
    ...
    results.forEach(result => {
      ...
      console.log(`   Text: ${result.textColor} | Background: ${result.backgroundColor}`);
      
      if (result.status === 'poor') {
        const minRatio = result.isLargeText ? WCAG_AA_LARGE : WCAG_AA_NORMAL;
        console.log(`   ⚠️  Needs ${minRatio}:1 minimum (current: ${result.ratio}:1)`);
      }
      console.log('');
    });
  }
  ...
  focusResults.forEach(result => {
    ...
    console.log(`   Ratio: ${result.ratio}:1 (${result.level})`);
    console.log(`   Focus: ${result.foreground} | Background: ${result.background}`);
    console.log('');
  });
  ...
  // Issues by theme
  console.log('\n🚨 ISSUES BY THEME:\n');
  const issuesByTheme = allResults.filter(r => r.status === 'poor')
    .reduce((acc, result) => {
...
function generateRecommendations(auditResults) {
  ...
  const criticalIssues = [...textResults, ...focusResults].filter(r => r.status === 'poor');
  
  if (criticalIssues.length > 0) {
    console.log('\n🚨 CRITICAL ISSUES (Immediate Action Required):\n');
    
    criticalIssues.forEach((issue, index) => {
      console.log(`${index + 1}. ${issue.combination || issue.type} - ${issue.theme || 'all themes'}`);
      ...
      if (issue.ratio < 2) {
        ...
        console.log(`   🎨 Minor adjustments needed - try one shade darker/lighter`);
      }
      console.log('');
    });
  }
  ...
  console.log('  - Use pure black (#000000) and white (#ffffff) more extensively');
  ...
  console.log('  - Ensure state colors (success/warning/error) are distinguishable');
  ...
  console.log('  - Purple brand colors may need adjustment for high contrast themes');
  console.log('  - Consider lighter purple shades for dark theme backgrounds\n');
...
function main() {
  ...
  console.log('💡 Run this audit regularly as you develop new components and themes.');
  console.log('=' .repeat(80));
}
