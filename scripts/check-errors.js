...
class ErrorChecker {
  ...
  async checkTypeScript() {
    ...
    const rootCheck = await this.runCommand("pnpm type-check", {
      silent: true,
    });
  ...
  async checkESLint() {
    ...
    if (existsSync(eslintReportPath)) {
      try {
        ...
        const errorCount = report.reduce(
        ...
        if (errorCount > 0) {
          ...
          this.log(
            `✅ packages/react: ESLint clean (${warningCount} warnings)`,
            "green",
          );
        }
      } catch (e) {
    ...
    if (!lintResult.success) {
      ...
      const matchErrs =
        text.match(/(\d+)\s+errors?/i) ||
        text.match(/problems\s*\((\d+)\s*errors?/i);
  ...
  async checkDesignTokens() {
    ...
    const missingBrands = requiredBrandFiles.filter(
  ...
  async generateReport() {
    ...
    Object.entries(errorsByType).forEach(([type, errors]) => {
      this.log(
        `\n${COLORS.bold}${type} Errors (${errors.length}):${COLORS.reset}`,
        "red",
      );
    ...
    if (errorsByType["ESLint"]) {
      this.log(
        "  • Run `pnpm lint:fix` to auto-fix ESLint issues (or `pnpm lint -- --fix`)",
        "white",
      );
    }
  ...
  async run() {
    this.log(
      "🔍 Starting automated error checking for ras-UI Design System...",
      "cyan",
    );
