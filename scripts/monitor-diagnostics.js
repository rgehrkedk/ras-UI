...
class DiagnosticsMonitor {
  ...
  async checkESLintReport() {
    ...
    if (!existsSync(eslintReportPath)) {
      this.log(
        "⚠️  ESLint report not found - run ESLint MCP to generate",
        "yellow",
      );
  ...
  generateErrorSummary(errors, warnings) {
    ...
    if (summary.totalErrors > 0) {
      summary.recommendations.push(
        "Run `pnpm lint --fix` to auto-fix ESLint errors",
      );
    }
    if (summary.criticalIssues.length > 0) {
      summary.recommendations.push(
        "Address critical issues in core component files immediately",
      );
    }
    ...
    if (Object.keys(summary.errorsByFile).length > 0) {
      ...
      Object.entries(summary.errorsByFile)
        ...
        .forEach(([file, stats]) => {
          ...
          this.log(
            `  ${indicator} ${file}: ${stats.errors} errors, ${stats.warnings} warnings`,
            "white",
          );
        });
    }
...
monitor
  ...
  .catch((error) => {
    console.error(
      `${COLORS.red}Diagnostics monitor failed:${COLORS.reset}`,
      error,
    );
    process.exit(1);
  });
